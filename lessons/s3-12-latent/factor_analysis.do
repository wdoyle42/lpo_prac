...
alpha `students' , item gen(student_scale) std

alpha `admins', item gen(admin_scale)

/*From the help page for alpha:  Unlike most Stata commands, generate() does not use casewise deletion.  A score is created for every observation for which there is a response to at least one item (one variable in varlist is not missing).  The summative score is divided by the number of items over which the sum is calculated.*/
