...
drop pred11 pred12 pred13 lb ub
...
graph twoway line pred12 pred13, ///
    xtitle("Hypothetical Values of Expenditures per Student (1000s)") ///
    ytitle("Predicted Values of Math Test Scores") ///
    name(basic_predict_margins_2)
...
graph twoway line pred12 pred13 || ///
	...
	// Shaded area type graphic

graph twoway rarea lb ub pred13, color(gs10) fintensity(inten20) || ///
	line pred12 pred13 , /// 
    xtitle("Hypothetical Values of Expenditures per Student (1000s) ") ///
    ytitle("Predicted Values of Math Test Scores") ///
    legend(order(1 "Predicted Value" 2 "Lower/Upper Bound 95% CI" )) ///
...
marginsplot ,name(margins1)

marginsplot , recast(line) plotopts(lcolor(black)) recastci(rarea)  name(margins2)
}
