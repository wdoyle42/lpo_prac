...
eststo zinb_full: zinb n_apps female `race' `pared' `test' `income' , inflate(female `race' `pared' `test' `income') zip
 
 
  // Marginal effects
  
  margins, dydx(female `race' `pared' `test' `income') atmeans
...
margins, predict(pr(`i')) at(byses=(`mymin'(`step')`mymax')  (min) female=(0 1) `race' `pared'  (mean) `test'  )  post
 
marginsplot, recastci(rarea) ciopts(color(%5)) recast(line) name(pry_`i', replace) xlabel(-2(.5)2)

graph save nb_`i'.gph, replace

}

grc1leg nb_1.gph nb_2.gph nb_3.gph nb_4.gph   
...
margins, predict(pr(2,100)) at((min) `race' `pared' /// 
								(mean) `test' `income' ///
								hispanic=(0 1) ///
								bynels2m=(`mymin'(`step')`mymax') /// 
								 ) 

marginsplot, recastci(rarea) ciopts(color(%5)) recast(line) name(more_than_2, replace) 
...
marginsplot, recastci(rarea) ciopt(color(%5)) recast(line) name(prymoren)
} // End count section
...
local step=`diff'/`no_steps'   
 ...
 margins, predict(pr(0,5000)) at(byses1=(`mymin'(`step')`mymax')  (min) female `race' `pared'  (mean) `test'  )  post
...
marginsplot, recastci(rarea) recast(line) name(ystar_credits, replace)
