...
$ttype, ///    // . means all in current memory
    not /// 
	
	
	// do not include t-tests 
...
graph hbar, over(bystexp byplans) asyvars
...
graph dot bynels2m bynels2r, over(bypared, ///
                         label(alternate ///
                               labsize(tiny) ///							   
                               ) ///
                         ) ///
...
graph bar plan_4, over(newpared)
