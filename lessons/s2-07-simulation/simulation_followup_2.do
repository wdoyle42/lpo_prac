...
forvalues i=1/`nreps'{
	...
	post buffer (_b[x]) // post the estimate to the buffer
...
if `inclass_example'==1{

clear 
 
local correlations .1 .25 .5 .75 
 
foreach i of local correlations{
 
mat newcol=(0.01\(`i')\-.02\.05\.05) /*Adds a column */

mat cormat2=cormat,newcol

mat cormat2=cormat2\0.01,`i',-.02,.05,.05,1 /*Adds a row */
...
gen y=`int'+(`fem_coeff'*female)+(`plans_coeff'*plans)+ ///
		(`race_coeff'*race)+(`pared_coeff'*pared)+(`ses_coeff'*ses) +  ///
		(`effect'*motivation)+e

keep y female plans race pared ses motivation e

save motivate_universe_2, replace

/*Now vary effect size*/

local effect_size 1 3 7 10

foreach effect of local effect_size{

use motivate_universe_2, clear
...
save motivate_universe_`effect'_`corr', replace

tempname results_store
postfile `results_store' plans1 plans2  using results_file_`effect'_`corr', replace
local j=1  
while `j'<=100{  

use motivate_universe_`effect'_`corr',clear  
...
use results_file_`effect'_`corr', clear

kdensity plans1, xline(`plans_coeff', lcolor(black) lpattern(dash)) /// 
		addplot(kdensity plans2) ///
		legend(order(1 "True Model" 2 "OVB Model")) ///
		title("Coefficient for Motivation=`effect', Correlation=`i'")
...
graph export monte_carlo_`corr'.pdf, replace
